...
module.exports = {
  ...
  category: "Reaction",
  usage: `bite para si mesmo\nbite @user para outra pessoa\nbite (respondendo mensagem)`,
  react: "😠",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} mordeu @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} se mordeu`;
        mentions = [user1];
      }
      ...
      console.log("Bite GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando bite:", error);
