...
module.exports = {
  name: "kick",
  alias: ["animekick"],
  desc: "To kick any user",
  category: "Reaction",
  usage: `kick para si mesmo\nkick @user para outra pessoa\nkick (respondendo mensagem)`,
  react: "👿",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} deu um chute em @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} deu um chute em si mesmo(a)`;
        mentions = [user1];
      }
      ...
      console.log("Kick GIF enviado com sucesso!\n");
