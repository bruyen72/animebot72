...
module.exports = {
  ...
  category: "Reaction",
  usage: `kill para si mesmo\nkill @user para outra pessoa\nkill (respondendo mensagem)`,
  react: "⚰️",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} eliminou @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} se eliminou`;
        mentions = [user1];
      }
      ...
      console.log("Kill GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando kill:", error);
