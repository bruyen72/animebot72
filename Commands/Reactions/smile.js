...
module.exports = {
  ...
  category: "Reaction",
  usage: `smile para si mesmo\nsmile @user para outra pessoa\nsmile (respondendo mensagem)`,
  react: "😸",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} sorriu para @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} sorriu para si mesmo(a)`;
        mentions = [user1];
      }
      ...
      console.log("Smile GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando smile:", error);
