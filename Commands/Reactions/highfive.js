...
module.exports = {
  ...
  category: "Reaction",
  usage: `highfive para si mesmo\nhighfive @user para outra pessoa\nhighfive (respondendo mensagem)`,
  react: "😎",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} deu um toca aqui com @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} deu um toca aqui consigo mesmo(a)`;
        mentions = [user1];
      }
      ...
      console.log("Highfive GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando highfive:", error);
