...
module.exports = {
  name: "cry",
  alias: ["animecry"],
  desc: "To cry for any user",
  category: "Reaction",
  usage: `cry para si mesmo\ncry @user para outra pessoa\ncry (respondendo mensagem)`,
  react: "😭",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} está chorando por @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} está chorando sozinho(a)`;
        mentions = [user1];
      }
      ...
      console.log("Cry GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando cry:", error);
