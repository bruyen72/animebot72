...
module.exports = {
  name: "happy",
  alias: ["animehappy"],
  desc: "To express happiness",
  category: "Reaction",
  usage: `happy para si mesmo\nhappy @user para outra pessoa\nhappy (respondendo mensagem)`,
  react: "😄",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} está feliz por ter @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} está feliz consigo mesmo(a)`;
        mentions = [user1];
      }
      ...
      console.log("Happy GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando happy:", error);
