...
module.exports = {
  name: "slap",
  alias: ["animeslap"],
  desc: "To slap someone",
  category: "Reaction",
  usage: `slap para si mesmo\nslap @user para outra pessoa\nslap (respondendo mensagem)`,
  react: "👊",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} deu um tapa em @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} deu um tapa em si mesmo(a)`;
        mentions = [user1];
      }
      ...
      console.log("Slap GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando slap:", error);
