...
module.exports = {
  ...
  category: "Reaction",
  usage: `handhold para si mesmo\nhandhold @user para outra pessoa\nhandhold (respondendo mensagem)`,
  react: "❤",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} e @${user2.split('@')[0]} estão de mãos dadas`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} está segurando as próprias mãos`;
        mentions = [user1];
      }
      ...
      console.log("Handhold GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando handhold:", error);
