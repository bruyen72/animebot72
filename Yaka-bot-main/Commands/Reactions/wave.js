...
module.exports = {
  ...
  category: "Reaction",
  usage: `wave para si mesmo\nwave @user para outra pessoa\nwave (respondendo mensagem)`,
  react: "👋",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} está acenando para @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} está acenando para si mesmo(a)`;
        mentions = [user1];
      }
      ...
      console.log("Wave GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando wave:", error);
