...
module.exports = {
  ...
  category: "Reaction",
  usage: `pat para si mesmo\npat @user para outra pessoa\npat (respondendo mensagem)`,
  react: "😇",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} acariciou @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} se acariciou`;
        mentions = [user1];
      }
      ...
      console.log("Pat GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando pat:", error);
