...
module.exports = {
  name: "yeet",
  alias: ["animeyeet"],
  desc: "To yeet someone",
  category: "Reaction",
  usage: `yeet para si mesmo\nyeet @user para outra pessoa\nyeet (respondendo mensagem)`,
  react: "☠️",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} arremessou @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} se arremessou`;
        mentions = [user1];
      }
      ...
      console.log("Yeet GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando yeet:", error);
