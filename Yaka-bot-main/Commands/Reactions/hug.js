...
module.exports = {
  ...
  category: "Reaction",
  usage: `hug para si mesmo\nhug @user para outra pessoa\nhug (respondendo mensagem)`,
  react: "🤭",
  start: async (Yaka, m, { text, mentionByTag, messageText, fullText }) => {
    try {
      ...
      if (user2) {
        caption = `@${user1.split('@')[0]} está abraçando @${user2.split('@')[0]}`;
        mentions = [user1, user2];
      } else {
        caption = `@${user1.split('@')[0]} está se abraçando`;
        mentions = [user1];
      }
      ...
      console.log("Hug GIF enviado com sucesso!\n");
      
    } catch (error) {
      console.error("ERRO no comando hug:", error);
